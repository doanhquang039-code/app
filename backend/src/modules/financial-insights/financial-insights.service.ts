...
export class FinancialInsightsService {
  ...
  async getHealthScore(userId: number) {
    ...
    score += savingsScore;
    if (savingsRate < 10) tips.push('Save at least 10% of monthly income');
    else if (savingsRate >= 20) tips.push('Great savings rate! Consider investing the surplus');
    ...
    score += expenseScore;
    if (expenseRatio > 0.9) tips.push('Expenses too high vs income, need to reduce spending');

    const trend = await this.getMonthlyTrend(userId, 3);
    const incomes = Object.values(trend).map((t: any) => t.income);
    const hasIncome = incomes.every((i) => i > 0);
    const incomeScore = hasIncome ? 20 : incomes.filter((i) => i > 0).length * 7;
    score += incomeScore;
    if (!hasIncome) tips.push('Record income regularly for better financial tracking');
    ...
    score += trackingScore;
    if (txCount < 10) tips.push('Record transactions more frequently for accurate analysis');
